...
const DYNAMIC_STRATEGIES = {
  momentum: {
    label: "듀얼 모멘텀",
    tip: "매 재평가 시점마다 후보로 추가한 자산 중 최근 [기준 기간] 수익률이 가장 높은 상위 [동시 보유 자산 수]개에 동일 비중으로 투자합니다. 선택된 자산의 수익률이 0% 이하면(절대모멘텀 미충족) 전액 현금성자산(BIL)으로 대피합니다. 입력한 비중 숫자는 사용하지 않습니다.",
    showTopN: true,
    usesWeightNumber: false,
  },
  trend: {
    label: "추세추종 (이동평균)",
    tip: "후보로 추가한 자산별로 입력한 비중을 기준비중(서로 간 상대 비율로 자동 환산)으로 삼아, 가격이 최근 [기준 기간] 이동평균선 위에 있으면 기준비중대로 보유하고 아래에 있으면 그만큼 현금성자산(BIL)으로 전환합니다.",
    showTopN: false,
